...
const (
    ...
    // Status operacionais (para notificações)
    StatusDisponivel          StatusMotorista = "disponivel"
    StatusOcupado            StatusMotorista = "ocupado"
    StatusOffline            StatusMotorista = "offline"
)
...
// Hooks do GORM para campos de timestamp
func (m *Motorista) BeforeCreate(tx *gorm.DB) error {
    now := time.Now()
    m.CriadoEm = now
    m.AtualizadoEm = now
    if m.Status == "" {
        m.Status = StatusAguardandoAprovacao
    }
    return nil
}

func (m *Motorista) BeforeUpdate(tx *gorm.DB) error {
...
// CalcularIdade calcula a idade baseada na data de nascimento
func (m *Motorista) CalcularIdade() int {
    agora := time.Now()
    idade := agora.Year() - m.DataNascimento.Year()
    
    // Ajusta se ainda não fez aniversário este ano
    if agora.YearDay() < m.DataNascimento.YearDay() {
        idade--
    }
...
// GetIdade retorna a idade (calculada ou armazenada)
func (m *Motorista) GetIdade() int {
    if m.Idade != nil {
        return *m.Idade
    }
    return m.CalcularIdade()
}
